...
The `model-provisioning` capability ensures the parakeet ONNX model files that the transcription model needs are present at `~/.config/steno/models/parakeet`, downloading them from HuggingFace when they are absent, so the daemon can transcribe audio without any manual, one-time setup step.
